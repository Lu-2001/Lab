...
//提示：url指代 （若干字符串）?（参数名1）=（参数1值）&（参数2）=（参数2值）...  这样的字符串。具体可以上网查找。例如：
// hjsdghgbj?name=666666&group=876。url、url_submit、url_result指代对应id的三个对象，其中url和url_result可以通过url.value或者url_result.value获得值。
//字符数组处理可以使用相关函数
...
function showWindowHref(){
    ...
    var num1 = str.indexOf("=",num);
    var num2 = str.indexOf("&",num);
    if((num !== -1) && (num1 !== -1) && (num2 !== -1)){
        url_result.value = str.substring((num1 + 1), num2);
    }
}
...
mul.value = 1;
let timerId = setInterval(timeTest,5000);

function timeTest(){
    let date1 = new Date();
    mul.value *= 2;
    if((mul.value >= 1024) || (date1.getSeconds() === 0)){
...
function arrSameStr(){
    let str = most.value, index = str[0];
    let nums = [];
    for (var i = 0; i < str.length; i++){
        nums[i] = 1;
    }

    nums.length = str.length;
